...
def test(
    ...
    seed=30,
    n_pedestrians=10,
    sumo_cfg=os.path.join("data","randomized","rand.sumocfg"),
    network=os.path.join("data","randomized","rand.net.xml"),
    bus_depot_start = '-269',
    bus_depot_end = '-269',
    bus_lane=0,
    n_buses=5,
    ...
    simulation = sim_cls(simulation_steps, 0, pedestrians, bus_depot_start, bus_depot_end,bus_lane=bus_lane, n_buses=n_buses)
    simulation.run()
...
with open(os.path.join("data","names","firstnames.txt")) as f:
    first_names = [name.strip() for name in f.readlines()]
with open(os.path.join("data","names","lastnames.txt")) as f:
...
if __name__ == '__main__':
    ...
    score = test(
        OptimizedFixedNBusesSimulation,
        bus_lane=1,
        gui=True,
        n_pedestrians=20,
        n_buses=2
